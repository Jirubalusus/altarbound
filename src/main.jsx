...
function ChoiceView({choice,game,onUnit,onHero,equipItem,train,finish}){
  if(choice.type==='training') return <div><h1>Training Grounds</h1><p>Select one of your units/heroes to improve. Disabled entries have no available upgrade.</p><div className="cards compact">{[...game.units,...game.heroes].map(c=><button key={c.uid} disabled={!canTrain(c)} className="pickCard" onClick={()=>train(c.uid)}><div className="bigIcon">{baseOf(c).icon}</div><h2>{fullName(c)} Lv{c.level}</h2><p>{trainingText(c)}</p></button>)}</div><button onClick={finish}>Skip</button></div>;
  if(choice.type==='item') return <div><h1>{choice.title}</h1><p>{choice.subtitle}</p><div className="cards compact">{choice.options.map((it,i)=><div className="pickCard" key={it.name}><div className="bigIcon">{it.icon}</div><h2>{it.name}</h2><p>{it.desc}</p><div className="equipGrid">{[...game.units,...game.heroes].map(c=><button key={c.uid} onClick={()=>equipItem(it,c.uid,'item')}>{baseOf(c).icon} {baseOf(c).name}</button>)}{game.heroes.map(h=><button key={h.uid+'2'} onClick={()=>equipItem(it,h.uid,'item2')}>{baseOf(h).icon} {baseOf(h).name} slot 2</button>)}</div></div>)}</div></div>;
  const opts=choice.options||[]; return <div><h1>{choice.title}</h1><p>{choice.subtitle}</p><div className="cards">{opts.map(o=>o.kind==='hero'?<HeroCard key={o.uid} h={o} onClick={()=>onHero(o)}/>:<UnitCard key={o.uid} u={o} onClick={()=>onUnit(o)} owned={game.units.some(x=>x.id===o.id)}/>)}</div><button onClick={finish}>Skip</button></div>
}
...
function UnitCard({u,onClick,owned}){ const b=UNITS[u.id], s=stats(u); return <button className="pickCard" onClick={onClick}><div className="bigIcon">{b.icon}</div><h2>{b.name}</h2><b>Lv. {u.level}</b><div className="tags"><span>{RACES[b.race]?.name||b.race}</span><span>{b.role}</span>{b.evolved&&<span>EVOLVED</span>}</div><div className="statgrid"><span>ATK {s.atk}</span><span>SPE {s.spd}</span><span>HP {u.maxHp}</span><span>ARM {s.armor}</span></div><p>{owned?'Duplicate: grants XP':b.ability}</p></button> }
function HeroCard({h,onClick}){ const b=HEROES[h.id], s=stats(h); return <button className="pickCard heroCard" onClick={onClick}><div className="bigIcon">{b.icon}</div><h2>{b.name}</h2><b>Lv. {h.level}</b><div className="tags"><span>{b.race}</span><span>Hero</span></div><div className="statgrid"><span>ATK {s.atk}</span><span>SPE {s.spd}</span><span>HP {h.maxHp}</span><span>ARM {s.armor}</span></div><p>{b.skills.join(' · ')}</p><p>Ultimate: {b.ultimate}</p></button> }
function ReplaceView({game,replaceEntity,cancel}){ const p=game.pendingReplace; const arr=p.kind==='unit'?game.units:game.heroes; return <div><h1>Roster full</h1><p>Replace one {p.kind} with <b>{baseOf(p.value).name}</b>, or skip.</p><div className="cards compact">{arr.map(c=><button className="pickCard" key={c.uid} onClick={()=>replaceEntity(c.uid)}><div className="bigIcon">{baseOf(c).icon}</div><h2>{baseOf(c).name}</h2><p>Lv{c.level} - replace this</p></button>)}</div><button onClick={cancel}>Cancel</button></div> }

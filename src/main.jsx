...
function ModelSprite({id, side='ally', small=false, title}){ const label=title||UNITS[id]?.name||HEROES[id]?.name||id; return <span className={`modelSprite ${side} ${small?'small':''}`} title={label}><img src={publicAsset(`war3-assets/models/${id}.png`)} alt="" onError={e=>{e.currentTarget.style.display='none'}}/><Portrait id={id} tiny title={label}/></span>; }

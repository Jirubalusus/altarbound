...
function firebaseHostedAsset(path){ return `${FIREBASE_HOSTING_ASSET_ROOT}${path}`; }
function Portrait({id, large=false, tiny=false, title}){ const race=(UNITS[id]?.race||HEROES[id]?.race||'neutral'); const label=title||UNITS[id]?.name||HEROES[id]?.name||id; const sources=[firebaseHostedAsset(`war3-assets/portraits/${id}.png`), publicAsset(`war3-assets/portraits/${id}.png`)]; const [srcIndex,setSrcIndex]=useState(0); useEffect(()=>setSrcIndex(0),[id]); const missing=srcIndex>=sources.length; return <span className={`wcPortrait ${race} ${large?'large':''} ${tiny?'tiny':''}`} title={label}>{!missing&&<img className="officialPortrait" src={sources[srcIndex]} alt="" onError={()=>setSrcIndex(i=>i+1)}/>}<i className="pixelHead" data-key={artKey(id)}/><em>{initials(label)}</em></span>; }
function ModelSprite({id, side='ally', small=false, title}){ const label=title||UNITS[id]?.name||HEROES[id]?.name||id; const sources=[firebaseHostedAsset(`war3-assets/models/${id}.png`), publicAsset(`war3-assets/models/${id}.png`), publicAsset(`sprites/models/${id}.png`)]; const [srcIndex,setSrcIndex]=useState(0); useEffect(()=>setSrcIndex(0),[id]); const missing=srcIndex>=sources.length; return <span className={`modelSprite ${side} ${small?'small':''} ${missing?'missingModel':''}`} title={label}>{!missing&&<img src={sources[srcIndex]} alt="" onError={()=>setSrcIndex(i=>i+1)}/>} {missing&&<Portrait id={id} tiny title={label}/>}</span>; }

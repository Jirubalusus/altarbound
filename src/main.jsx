...
const FIREBASE_STORAGE_BUCKET = 'altarbound-660da.firebasestorage.app';
const FIREBASE_ASSET_ROOT = `https://firebasestorage.googleapis.com/v0/b/${FIREBASE_STORAGE_BUCKET}/o/`;
function firebaseStorageAsset(path){ return `${FIREBASE_ASSET_ROOT}${encodeURIComponent(path)}?alt=media`; }
function Portrait({id, large=false, tiny=false, title}){ const race=(UNITS[id]?.race||HEROES[id]?.race||'neutral'); const label=title||UNITS[id]?.name||HEROES[id]?.name||id; const sources=[firebaseStorageAsset(`war3-assets/portraits/${id}.png`), publicAsset(`war3-assets/portraits/${id}.png`)]; const [srcIndex,setSrcIndex]=useState(0); useEffect(()=>setSrcIndex(0),[id]); const missing=srcIndex>=sources.length; return <span className={`wcPortrait ${race} ${large?'large':''} ${tiny?'tiny':''}`} title={label}>{!missing&&<img className="officialPortrait" src={sources[srcIndex]} alt="" onError={()=>setSrcIndex(i=>i+1)}/>}<i className="pixelHead" data-key={artKey(id)}/><em>{initials(label)}</em></span>; }
function ModelSprite({id, side='ally', small=false, title}){ const label=title||UNITS[id]?.name||HEROES[id]?.name||id; const sources=[firebaseStorageAsset(`war3-assets/models/${id}.png`), publicAsset(`war3-assets/models/${id}.png`), publicAsset(`sprites/models/${id}.png`)]; const [srcIndex,setSrcIndex]=useState(0); useEffect(()=>setSrcIndex(0),[id]); const missing=srcIndex>=sources.length; return <span className={`modelSprite ${side} ${small?'small':''} ${missing?'missingModel':''}`} title={label}>{!missing&&<img src={sources[srcIndex]} alt="" onError={()=>setSrcIndex(i=>i+1)}/>} {missing&&<Portrait id={id} tiny title={label}/>}</span>; }
...
function RaceSelect({onPick}){ return <div className="page"><h1>Choose Your Race!</h1><div className="cards">{Object.entries(RACES).map(([id,r])=><button className="pickCard" key={id} onClick={()=>onPick(id)}><ModelSprite id={r.starter} small title={UNITS[r.starter].name}/><h2 style={{color:r.color}}>{r.name}</h2><p>Starter: {UNITS[r.starter].name}</p><p>{r.style}</p></button>)}</div></div> }

...
function UnitCard({u,onClick,owned}){ const b=UNITS[u.id], s=stats(u); return <button className="pickCard" onClick={onClick}><Portrait id={u.id} large title={b.name}/><h2>{b.name}</h2><b>Lv. {u.level}</b><div className="tags"><span>{RACES[b.race]?.name||b.race}</span><span>{b.role}</span>{b.evolved&&<span>EVOLVED</span>}</div><div className="statgrid"><span>ATK {s.atk}</span><span>SPE {s.spd}</span><span>HP {u.maxHp}</span><span>ARM {s.armor}</span></div><p>{owned?'Duplicate: grants XP':b.ability}</p></button> }
function HeroCard({h,onClick}){ const b=HEROES[h.id], s=stats(h); return <button className="pickCard heroCard" onClick={onClick}><Portrait id={h.id} large title={b.name}/><h2>{b.name}</h2><b>Lv. {h.level}</b><div className="tags"><span>{b.race}</span><span>Hero</span></div><div className="statgrid"><span>ATK {s.atk}</span><span>SPE {s.spd}</span><span>HP {h.maxHp}</span><span>ARM {s.armor}</span></div><p>{b.skills.join(' · ')}</p><p>Ultimate: {b.ultimate}</p></button> }
function ReplaceView({game,replaceEntity,cancel}){ const p=game.pendingReplace; const arr=p.kind==='unit'?game.units:game.heroes; return <div><h1>Roster full</h1><p>Replace one {p.kind} with <b>{baseOf(p.value).name}</b>, or skip.</p><div className="cards compact">{arr.map(c=><button className="pickCard" key={c.uid} onClick={()=>replaceEntity(c.uid)}><Portrait id={c.id} large title={baseOf(c).name}/><h2>{baseOf(c).name}</h2><p>Lv{c.level} - replace this</p></button>)}</div><button onClick={cancel}>Cancel</button></div> }
...
function BattleCard({c,enemy}){ const b=baseOf(c); const hp=Math.max(0,100*c.hp/c.maxHp); const s=stats(c); return <div className={`battleCard ${enemy?'enemy':''} ${c.hp<=0?'dead':''}`}><div className="nameLine"><b><Portrait id={c.id} tiny title={b.name}/> {b.name} Lv{c.level}</b><span>{c.hp}/{c.maxHp}</span></div><div className="hp"><i style={{width:`${hp}%`}}/></div><div className="speed"><i style={{width:`${Math.min(100,c.speed)}%`}}/></div>{c.kind==='hero'&&<div className="power"><i style={{width:`${Math.min(100,100*c.power/powerMax(c))}%`}}/><small>{selectedSkillName(c)}</small></div>}<small>ATK {s.atk} ARM {s.armor} SPE {s.spd} {c.item&&` · ${c.item.name}`}</small></div> }

...
const nodeAssetMetrics = await page.evaluate(() => {
  ...
  return {
    ...
    nonValidatedSources: imgs.map(img=>img.src).filter(src=>!(src.includes('/sprites/battle.png') || src.includes('/sprites/elite.png') || src.includes('/sprites/boss.png') || src.includes('/hive-assets/nodes/') || src.includes('/hive-assets/nodes-readable/'))),
    ...
    nonGenericBattleNodes: nodes.filter(n=>n.classList.contains('battle')).map(n=>n.querySelector('.nodeWar3Asset')?.src || '').filter(src=>!src.includes('/sprites/battle.png') || /grunt|footman|archer|ghoul|character-faces|portraits/.test(src)).length,
    ...
    nonSymbolDangerNodes: nodes.filter(n=>['elite','boss'].some(cls=>n.classList.contains(cls))).map(n=>n.querySelector('.nodeWar3Asset')?.src || '').filter(src=>!(src.includes('/sprites/elite.png') || src.includes('/sprites/boss.png')) || /character-faces|portraits/.test(src)).length,
...
if (nodeAssetMetrics.war3Assets !== nodeAssetMetrics.nodes || nodeAssetMetrics.oldSpriteIcons !== 0 || nodeAssetMetrics.oldTopOverlayVisible || !nodeAssetMetrics.routeLegendVisible || !nodeAssetMetrics.usesWowMapBackground || nodeAssetMetrics.nonValidatedSources.length || nodeAssetMetrics.illogicalItemNodes || nodeAssetMetrics.nonGenericBattleNodes || nodeAssetMetrics.nonSemanticSiteNodes || nodeAssetMetrics.nonSymbolDangerNodes || nodeAssetMetrics.nonHiveObjectNodes || nodeAssetMetrics.hiddenFutureAssets || nodeAssetMetrics.generatedNodeChrome) throw new Error(`Map nodes must use semantic generic route icons/sites on a WoW route background with no generated circles/peanas or confusing unit portraits: ${JSON.stringify(nodeAssetMetrics)}`);

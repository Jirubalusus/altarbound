...
if (!raceGruntModel.includes('/generated-assets/units/orc-grunt-pro-pokelike-facing-right.webp')) throw new Error(`Race select Grunt must use the generated full-body Grunt model, not a face portrait: ${raceGruntModel}`);
...
const nodeAssetMetrics = await page.evaluate(() => {
  ...
  return {
    ...
    nonValidatedSources: imgs.map(img=>img.src).filter(src=>!(src.includes('/sprites/battle-crossed-swords.png') || src.includes('/sprites/elite-crossed-swords-crown.png') || src.includes('/hive-assets/nodes/') || src.includes('/hive-assets/nodes-readable/') || src.includes('/wow-assets/character-faces/'))),
    ...
    nonGenericBattleNodes: nodes.filter(n=>n.classList.contains('battle')).map(n=>n.querySelector('.nodeWar3Asset')?.src || '').filter(src=>!src.includes('/sprites/battle-crossed-swords.png') || /grunt|footman|archer|ghoul|character-faces|portraits/.test(src)).length,
    nonSemanticSiteNodes: nodes.filter(n=>['tavern','altar','special','training','tower'].some(cls=>n.classList.contains(cls))).map(n=>n.querySelector('.nodeWar3Asset')?.src || '').filter(src=>!(src.includes('/hive-assets/nodes-readable/')) || /headhunter|troll|grunt|footman|character-faces|portraits/.test(src)).length,
    nonSymbolDangerNodes: nodes.filter(n=>n.classList.contains('elite')).map(n=>n.querySelector('.nodeWar3Asset')?.src || '').filter(src=>!src.includes('/sprites/elite-crossed-swords-crown.png') || /character-faces|portraits/.test(src)).length,
    nonHeroBossNodes: nodes.filter(n=>n.classList.contains('boss')).map(n=>n.querySelector('.nodeWar3Asset')?.src || '').filter(src=>!(/\/wow-assets\/character-faces\/(paladin|far_seer|demon_hunter|death_knight|dark_ranger)\.png/.test(src))).length,
...
if (nodeAssetMetrics.war3Assets !== nodeAssetMetrics.nodes || nodeAssetMetrics.oldSpriteIcons !== 0 || nodeAssetMetrics.oldTopOverlayVisible || !nodeAssetMetrics.routeLegendVisible || !nodeAssetMetrics.usesWowMapBackground || nodeAssetMetrics.nonValidatedSources.length || nodeAssetMetrics.illogicalItemNodes || nodeAssetMetrics.nonGenericBattleNodes || nodeAssetMetrics.nonSemanticSiteNodes || nodeAssetMetrics.nonSymbolDangerNodes || nodeAssetMetrics.nonHeroBossNodes || nodeAssetMetrics.nonHiveObjectNodes || nodeAssetMetrics.hiddenFutureAssets || nodeAssetMetrics.generatedNodeChrome) throw new Error(`Map nodes must use semantic route icons: crossed swords for Battle, crossed swords+crown for Elite, race final hero portrait for Boss, sites for non-combat nodes, with no generated circles/peanas or confusing unit portraits: ${JSON.stringify(nodeAssetMetrics)}`);
...
const battleMetrics = await page.evaluate(() => ({
  ...
  allyGruntTransform: (() => { const img = document.querySelector('.allySide .arenaStage .modelSprite[title="Grunt"] img'); return img ? getComputedStyle(img).transform : 'missing'; })(),
  enemyGruntTransform: (() => { const img = document.querySelector('.enemySide .arenaStage .modelSprite[title="Grunt"] img'); return img ? getComputedStyle(img).transform : 'missing'; })(),
  platformCount: document.querySelectorAll('.pokelikeBattle .arenaStage').length
}));
...
if (!battleMetrics.gruntModelSrc.includes('/generated-assets/units/orc-grunt-pro-pokelike-facing-right.webp')) throw new Error(`Battle Grunt must fight as the generated full-body model: ${JSON.stringify(battleMetrics)}`);
if (battleMetrics.allyGruntTransform.startsWith('matrix(-') || battleMetrics.platformCount < 2) throw new Error(`Battle orientation/platforms are not Pokelike-facing: ${JSON.stringify(battleMetrics)}`);
